...
Command111.CommandText = "INSERT INTO 留言 (账号,IP, 类型, 时间)  VALUES ('"& session("MM_username") & "',' "  & request.servervariables("remote_addr")  & "','information','" & year(Now) & "-" & Month(Now) & "-" & day(Now) & " " & Hour(Now) & ":" & Minute(Now) & ":" & Second(Now) & "' ) "
...
<div class="port"  style="font:'楷体'; min-height:100px">
  <h3>个人信息</h3>
		<p class="para">在此页面您可以查看自己的个人信息，并在管理员开放修改权限时修改信息。<span></span></p>
  <img src="hr-geo-sukarno-1s960x639.png"  style="  position:relative; left:2%;background-size:cover; background-position:center; width:90%;height:90%" alt=""/>
    ...
    <tr class="altrow">                  <!-- 奇数行 -->  
     ...
     <tr>                  <!-- 奇数行 -->  
        ...
        <td><%=(Recordset1.Fields.Item("身份证号").Value)%></td>  
    </tr>  
   
      <tr>                  <!-- 奇数行 -->  
        <td><i class="glyphicon glyphicon-tree-conifer"></i>教学班</td>  
        <td><%=(Recordset1.Fields.Item("教学班").Value)%></td>  
        ...
        <td><%=(Recordset1.Fields.Item("培养方向").Value)%></td>  
                   ...
                   <p align="center">&nbsp;</p>
  <p align="center">————————————————————————————————————————————————————————</p>
  <p align="center" style="font-size:14px">注意：信息修改权限此时并未开放，若有问题请联系管理员。</p>
  <p align="center">————————————————————————————————————————————————————————</p>
  <p align="center">&nbsp;</p>
          </div>

...
Recordset6_cmd.CommandText = "SELECT * FROM student.applyend WHERE  sno = ? and type = '协议返回'" 
...
Command1.CommandText = "INSERT INTO student.applyend (sno, time, type, state,name,telephone)  VALUES ('"&Session("MM_Username")&"','"&date() &" " & time()&"','协议返回','未处理','"& Recordset1.Fields.Item("姓名").Value &"','"&request.Form("tel")&"' ) "
...
%>
          ...
          <p  style="font-size:18px"align="center"><span class="portfolio-grids" style="font-size:18px; font-family:'微软雅黑'"><%=(Recordset1.Fields.Item("姓名").Value)%>同学</span>，三方协议返回预约成功，预约时间为<%=date()&" "&time()%>。</p>
          <% if datediff("s",date() & " " & "12:40:00",now()) <= 0 then %>
  <p  style="font-size:18px"align="center">您预约的时间在12点40分之前，请您在<strong><%=(Recordset4.Fields.Item("today").Value)%>（<%=(Recordset4.Fields.Item("weektoday").Value)%>）</strong>的12点50分去导员办公室送三方协议。</p>
  <p  style="font-size:18px"align="center">三方协议返回时，请将自己的三方协议自行放在<strong>导员办公室门口的办公桌上</strong>，并在当时填写就业信息的本子的<strong style="color:blue">“协议返回情况”</strong>上打钩，不需要等待人来处理。</p>
  <%else%>
  <p  style="font-size:18px"align="center">您预约的时间在12点40分之后，请您在下一个工作日的中午12：50，即<strong><%=(Recordset4.Fields.Item("tomorrow").Value)%>（<%=(Recordset4.Fields.Item("weektomorrow").Value)%>）</strong>的12点50分去导员办公室送三方协议。</p>
  <p  style="font-size:18px"align="center">三方协议返回时，请将自己的三方协议自行放在<strong>导员办公室门口的办公桌上</strong>，并在当时填写就业信息的本子的<strong style="color:blue">“协议返回情况”</strong>上打钩，不需要等待人来处理。</p>
  <%end if%>
  <p  style="font-size:18px"align="center">&nbsp;</p>
  <p>
    <%else%>
    <%=(Recordset1.Fields.Item("姓名").Value)%>同学，你已经于<%=(Recordset6.Fields.Item("time").Value)%>在本系统预约过三方协议返回，申请状态是：<%=(Recordset6.Fields.Item("state").Value)%>，你不能再次在本系统预约协议返回。</p>
  <p>
...
{
 window.location='applyreturn.asp'

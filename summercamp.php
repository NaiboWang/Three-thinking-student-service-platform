...
<div class="typrography">
	 <div class="container">
     <p  align="center" class = "yyjh" style="display:none; font-size:23px ">各学校夏令营/优研计划汇总</p>
	      <img  class="xlytp" src="\images\xly.png" style="background-position:center;left:14%;position:relative;background-size:cover; min-height:50px; min-width:200px; width:70%;height:80%" alt=""/>
	
	   <div class="clearfix"> </div>
</div>
<div class="container" style="margin-left:5%; width:90%; margin-top:15px">
			 <div class="grid_3 grid_5" >
				 <h3>排序方式<span class="bjsj" style="float:right;text-align:right;display:block" id=time>?</span></h3>
...
show();
			...
			<ul id="myTab" class="nav nav-tabs" role="tablist">
              <li role="presentation" <?php if($_GET['lx'] == 1) {?> class="active" <?php } ?> ><a href="summercamp.php?lx=1&sx=<?php echo $_GET['sx'] ?>" id="home-tab" role="tab" aria-controls="home" aria-expanded="true">发布时间</a></li>
			  <li role="presentation" <?php if($_GET['lx'] == 2) {?> class="active" <?php } ?> ><a href="summercamp.php?lx=2&sx=<?php echo $_GET['sx'] ?>" id="home-tab" role="tab"  aria-controls="home" aria-expanded="true">大学</a></li>
			  <li role="presentation" <?php if($_GET['lx'] == 3) {?> class="active" <?php } ?> ><a href="summercamp.php?lx=3&sx=<?php echo $_GET['sx'] ?>" role="tab" id="profile-tab"  aria-controls="profile">材料投递截止时间</a></li>
			  <li role="presentation"  class="dropdown">
				<a href="#" id="myTabDrop1" class="dropdown-toggle" data-toggle ="dropdown"  aria-controls="myTabDrop1-contents"><?php if($_GET['sx'] == 1) {?>筛选：直硕<?php } elseif($_GET['sx'] == 2) {?> 筛选：纯直博<?php } elseif($_GET['sx'] == 3) {?> 筛选：优研<?php } elseif($_GET['sx'] == 4) {?> 筛选：其他<?php } else {?> 类型：全部<?php }?><span class="caret"></span></a>
				<ul class="dropdown-menu" role="menu" aria-labelledby="myTabDrop1" id="myTabDrop1-contents">
				  <li <?php if($_GET['sx'] == 1) {?> class="active" <?php } ?> ><a href="summercamp.php?lx=<?php echo $_GET['lx'] ?>&sx=1" tabindex="-1" role="tab" id="dropdown1-tab"  aria-controls="dropdown1">直硕</a></li>
				  <li <?php if($_GET['sx'] == 2) {?> class="active" <?php } ?> ><a href="summercamp.php?lx=<?php echo $_GET['lx'] ?>&sx=2" tabindex="-1" role="tab" id="dropdown2-tab" aria-controls="dropdown2">纯直博</a></li>
                  	  <li <?php if($_GET['sx'] == 3) {?> class="active" <?php } ?> ><a href="summercamp.php?lx=<?php echo $_GET['lx'] ?>&sx=3" tabindex="-1" role="tab" id="dropdown3-tab"  aria-controls="dropdown3">优研</a></li>
				  <li <?php if($_GET['sx'] == 4) {?> class="active" <?php } ?> ><a href="summercamp.php?lx=<?php echo $_GET['lx'] ?>&sx=4" tabindex="-1" role="tab" id="dropdown4-tab" aria-controls="dropdown4">其他</a></li>
                   <li <?php if($_GET['sx'] == 0) {?> class="active" <?php } ?> ><a href="summercamp.php?lx=<?php echo $_GET['lx'] ?>&sx=0" tabindex="-1" role="tab" id="dropdown4-tab" aria-controls="dropdown4">全部</a></li>
...
$str2 = "select month(max(pubdate)) as month,day(max(pubdate)) as day from summercamp";
$result = mysql_query($str2,$conn); //查询
$rst = mysql_fetch_array($result);
 ?>
            <h3>夏令营信息，最新更新时间：<?php echo $rst['month']."月",$rst['day']."日" ?></h3>
            ...
            <table class="datatable">
              <thead>
                <tr >
                  ...
                  <th>大学</th>
                  <th>类型</th>
                  <th class="xt2">材料投递截止时间<br>(悬停可看具体日期)</br></th>
                  <th class="xt" style="display:none">材料投递截止时间</th>
                  <th>发布时间</th>
                  <th>状态</th>
                </tr>
...
　} else {
	...
	$( "th.xt" ).slideToggle( 3, function() {});
	$( "th.xt2" ).slideToggle( 3, function() {});
　　}
　　
</script>
   <?php
...
{
	$_SESSION['name']='ceshi';
	$str2 = "update num set num = num + 1";
mysql_query($str2); 
	}

	  

$str2 = "insert into recordxly(time,type,IP) values('".date('y-m-d h:i:s',time())."','". $_GET['lx']." ".$_GET['sx']."','".$_SERVER["REMOTE_ADDR"]."')";
mysql_query($str2); //数据库输出编码 应该与你的数据库编码保持一致.南昌网站建设公司百恒网络PHP工程师建议用UTF-8 国际标准编码.

$str1 = "select * from summercamp ";
if($_GET['sx'] == 1)
$str1 = $str1."where find_in_set('直硕',type) ";
elseif($_GET['sx'] == 2)
$str1 = $str1."where type = '直博' ";
elseif($_GET['sx'] == 3)
$str1 = $str1."where find_in_set('优研',type) ";
elseif($_GET['sx'] == 4)
$str1 = $str1."where find_in_set('纯粹的交流',type) ";

...
Command111.CommandText = "INSERT INTO 留言 (账号,IP, 类型, 时间)  VALUES ('"& session("MM_username") & "',' "  & request.servervariables("remote_addr")  & "','password','" & year(Now) & "-" & Month(Now) & "-" & day(Now) & " " & Hour(Now) & ":" & Minute(Now) & ":" & Second(Now) & "' ) "
				...
				<div id="form_wrapper" class="form_wrapper">
					<form onSubmit="return takein()" action="alterpass.asp" method="post" class="register active" name="changepass">
						...
						<div class="column">
						  <div>
						    <label>学号：</label>
						    <input type="text" value="<%=session("MM_username")%>" readonly/>
						     </div>
						  <div> 						    
                            <label>旧密码：</label> 						    

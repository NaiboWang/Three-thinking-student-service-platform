...
<img src="hr-geo-sukarno-1s960x639.png"  style="  position:relative; left:3%;background-size:cover; background-position:center; width:90%;height:90%" alt=""/>
     ...
     <div align="center" style="width:75%">
          ...
          <p  style="font-size:18px"align="center"><span class="portfolio-grids" style="font-size:18px; font-family:'微软雅黑'"><%=(Recordset1.Fields.Item("姓名").Value)%>同学</span>，在预约三方协议返回前，请确保协议上的所有内容都已经正确和完整，然后在办公时间把其中一份交到导员办公室。</p>
  <p  style="font-size:18px"align="center">协议返回每位同学只能<strong>预约一次</strong>，如果你考虑好了，就在下方填写手机号，并点击下方按钮进行预约。  </p>
  <form name="form1" method="post" action="applyreturndeal.asp">
    <p>
  ...
  <p>
    <%else%>
    <%=(Recordset1.Fields.Item("姓名").Value)%>同学，你已经于<%=(Recordset6.Fields.Item("time").Value)%>在本系统预约过三方协议返回申请，申请状态是：<strong><%=(Recordset6.Fields.Item("state").Value)%></strong>，你不能再次在本系统预约返回。</p>
  <p><%if Recordset6.Fields.Item("state").Value = "未处理" then%>
  ------------------------------------------------------------------------------------------
   <% if datediff("s",date() & " " & "12:40:00",now()) <= 0 then %>
  <p  style="font-size:18px"align="center">请你在<strong><%=(Recordset4.Fields.Item("today").Value)%>（<%=(Recordset4.Fields.Item("weektoday").Value)%>）</strong>的12点50分去导员办公室送三方协议。</p>
<p  style="font-size:18px"align="center">三方协议返回时，请将自己的三方协议自行放在<strong>导员办公室门口的办公桌上</strong>，并在当时填写就业信息的本子的<strong style="color:blue">“协议返回情况”</strong>上打钩，不需要等待人来处理。</p>
  <%else%>
  <p  style="font-size:18px"align="center">请你在下一个工作日的中午12：50，即<strong><%=(Recordset4.Fields.Item("tomorrow").Value)%>（<%=(Recordset4.Fields.Item("weektomorrow").Value)%>）</strong>的12点50分去导员办公室送三方协议。</p>
  <p  style="font-size:18px"align="center">三方协议返回时，请将自己的三方协议自行放在<strong>导员办公室门口的办公桌上</strong>，并在当时填写就业信息的本子的<strong style="color:blue">“协议返回情况”</strong>上打钩，不需要等待人来处理。</p>
  <%end if%>
  ------------------------------------------------------------------------------------------
<%end if%>

...
Recordset3_cmd.CommandText = "SELECT * FROM student.countinfor WHERE ID = ?"
...
Recordset4_cmd.CommandText = "SELECT 学号, 姓名 FROM student.身份信息（不可泄露） WHERE 教学班 ='"&(Recordset6.Fields.Item("privilege").Value)&"' and 学号 not in(select 学号 from countable"&request.Form("ID")&")" 
...
<img src="hr-geo-sukarno-1s960x639.png"  style="  position:relative; left:3%;background-size:cover; background-position:center; width:90%;height:90%" alt=""/>
		...
		<div  align="center"  class="portfolio-grids" style="font-size:18px; font-family:'微软雅黑'">
          <% If Not Recordset6.EOF Or Not Recordset6.BOF Then %>
  <h2  style="font-size:18px"align="center">尚未提交统计学生名单</h2>

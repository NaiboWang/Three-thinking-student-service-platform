...
$sql22 ="select * from 加权平均分 where 学号 = '" . $xh . "'"; //SQL语句
...
$str1 =  "		兹证明，". $row['姓名']."，".$row['性别']."，身份证号".$row['身份证号']."，系我院".$sh['大方向']."专业".$row['教学班']."班学生，学号".$row['学号']."，大学期间主干课程加权平均分为：".$row2['加权平均分']."，排名：".$row2['位次']."/".$sh['人数'].$xfx."。";
...
if($sh['前五学期'] == '1')//前五学期说明
	$pdf->Write(10,"		该排名所用成绩截至第五学期，仅限于申请夏令营使用，特此说明。",'', 0, 'L', true, 0, false, false, 0); 
$pdf->Ln(16);
...
$pdf->Write(10,"成绩证明真实性校验码(防止伪造成绩排名)：".($sh['人数'] -$row2['位次']) % (substr(date("d",$time),1,1)+10),'', 0, 'L', true, 0, false, false, 0); 
//输出PDF 

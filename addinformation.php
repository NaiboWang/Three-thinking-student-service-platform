...
$ok = 1;
  for ($i= 2;$i<= count($res); $i++){ 
      $str= $res[$i]; 
      $sql = "insert into 身份信息（不可泄露） (年级,学号,姓名,教学班,身份证号,培养方向) values('{$str[0]}','{$str[1]}','{$str[2]}','{$str[3]}','{$str[4]}','{$str[5]}')";
...
if($ok)
{
	mysqli_query($link,"call 更新基本信息()"); //首先删除已经存在的表信息
	mysqli_commit($link);
	mysqli_autocommit($link,true); // 开始自动commit
	echo "<script>
alert('提交成功！');
 window.location.href='uploadpre.html';</script>";//跳转到提交成功页面
	}
	else
	{
		echo "<script>

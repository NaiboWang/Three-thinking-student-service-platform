...
R7_cmd.CommandText = "SELECT news FROM 统计" 
...
Set R8_cmd = Server.CreateObject ("ADODB.Command")

R8_cmd.ActiveConnection = MM_tj_STRING
R8_cmd.CommandText = "SELECT count(*) as num FROM 留言" 
R7_cmd.Prepared = true
Set R8 = R8_cmd.Execute
R8_numRows = 0
...
<div align="center">
  ...
  <p align="left">导师页面访问次数：<%=(R7.Fields.Item("news").Value)%>次</p>
  <p align="left"><a href="admin.php?lx=1485242" target="_blank">mysql管理</a></p>
  <div  style="overflow:scroll; height:1000px; width: 900px;">
  ...
  <% 
  if R8.Fields.Item("num").value <300 then
  a = 0
  else
  a = R8.Fields.Item("num").value - 300
  end if
...
%>
    <table width="400" border="1">
      <tr>
        ...
        <td nowrap="nowrap"><%=(Recordset6.Fields.Item("账号").Value)%></td>

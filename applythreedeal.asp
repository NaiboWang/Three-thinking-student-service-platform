...
Command1.CommandText = "INSERT INTO student.applyend (sno, time, type, state,name,telephone)  VALUES ('"&Session("MM_Username")&"','"&date() &" " & time()&"','三方协议','未处理','"& Recordset1.Fields.Item("姓名").Value & "','"&request.Form("tel") &"') "
Command1.CommandType = 1
Command1.CommandTimeout = 0
Command1.Prepared = true
Command1.Execute()
Command1.CommandText = "update student.applythree set 学籍所在地 = '"&request.Form("hometown")&"',提交 = 1,接受单位名称 = '"&request.Form("company")&"',户口是否在校 = '"&request.Form("radio")&"',双选会 = '"&request.Form("sxh")&"' where 学号 = '"&Session("MM_Username")&"'"
Command1.Execute()
%>

          <p  style="font-size:18px"align="center"><span class="portfolio-grids" style="font-size:18px; font-family:'微软雅黑'"><%=(Recordset1.Fields.Item("姓名").Value)%>同学</span>，三方协议盖章预约成功，预约时间为<%=date()&" "&time()%>。</p>
          <% if datediff("s",date() & " " & "12:40:00",now()) <= 0 then %>
  <p  style="font-size:18px"align="center">您预约的时间在12点40分之前，请您在<strong><%=(Recordset4.Fields.Item("today").Value)%>（<%=(Recordset4.Fields.Item("weektoday").Value)%>）</strong>的12点50分带好<strong>四份协议和相关材料</strong>去导员办公室盖章。</p>
  <p  style="font-size:18px"align="center">办公时间为：<strong style="color:#00F">12：50——13：30</strong>，<strong>过期不候</strong>，请按时领取。</p>
  <%else%>
  <p  style="font-size:18px"align="center">您预约的时间在12点40分之后，请您在下一个工作日的中午12：50，即<strong><%=(Recordset4.Fields.Item("tomorrow").Value)%>（<%=(Recordset4.Fields.Item("weektomorrow").Value)%>）</strong>的12点50分带好<strong>四份协议和相关材料</strong>去导员办公室盖章。</p>
  <p  style="font-size:18px"align="center">办公时间为：<strong style="color:#00F">12：50——13：30</strong>，<strong>过期不候</strong>，请按时领取。</p>
  <%end if%>
  <p  style="font-size:18px"align="center">&nbsp;</p>
  <p>
    <%else%>
    <%=(Recordset1.Fields.Item("姓名").Value)%>同学，你已经于<%=(Recordset6.Fields.Item("time").Value)%>在本系统预约过三方协议盖章，申请状态是：<%=(Recordset6.Fields.Item("state").Value)%>，你不能再次在本系统预约三方协议盖章。</p>
  <p>
...
{
 window.location='applythree.asp'
